...
const startingBearer = process.env.BEARER // The bearer that is in the .env file when the bot starts
let Bearer = process.env.BEARER // The bearer that will be updated if it's invalid
...
async function updateBearer() {
    try {
        ...
        const GetNewBearerReq = await axios.post(`https://id.twitch.tv/oauth2/token?client_id=${process.env.CLIENTID}&client_secret=${process.env.CLIENTSECRET}&grant_type=client_credentials`)
